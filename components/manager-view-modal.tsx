...
import { UserCog, MessageSquare, BarChart3, FileSpreadsheet, FileText } from "lucide-react"
import type { SurveyResponse } from "../types/survey"
import { getScoreDescription, calculateSectionScores, calculateOverallScore } from "../lib/score-utils"
import { MANAGER_SECTIONS } from "../lib/survey-data"

interface ManagerViewModalProps {
...
export function ManagerViewModal({ submission, isOpen, onClose }: ManagerViewModalProps) {
  ...
  const handleExport = (format: "csv" | "json") => {
    ...
    if (format === "csv") {
      const csvContent = [
        ...
        ...Object.entries(submission.responses).flatMap(([sectionId, sectionResponses]) =>
          Object.entries(sectionResponses).map(([questionId, response]) => {
            const section = MANAGER_SECTIONS.find((s) => s.id === sectionId)
      ...
      link.setAttribute("download", `manager-assessment-${submission.managerName?.replace(/\s+/g, "-")}.csv`)
      ...
      const url = URL.createObjectURL(blob)
      link.setAttribute("href", url)
      link.setAttribute("download", `manager-assessment-${submission.managerName?.replace(/\s+/g, "-")}.json`)
      link.style.visibility = "hidden"
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
  ...
  const sectionScores = calculateSectionScores(submission.responses, MANAGER_SECTIONS)
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle className="text-xl">Manager Assessment - {submission.managerName}</DialogTitle>
        ...
        <Tabs defaultValue="analytics" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="comments">Manager Feedback</TabsTrigger>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg flex items-center">
                  ...
                  Overall Manager Assessment Score
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-600">
                  Additional feedback and recommendations from the manager assessment
                </p>
              </CardHeader>
              <CardContent>
                {submission.comments ? (
          ...
          <TabsContent value="export" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Export Manager Assessment Data</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <p className="text-sm text-gray-600">
                  Export the manager assessment results for {submission.managerName} including responses and feedback.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
